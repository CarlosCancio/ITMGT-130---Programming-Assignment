...
def change_password():
    if request.method == 'POST':
        ...
        if user and user["password"] == old_password:
            ...
            db.update_one({"email": user_email}, {"$set": {"password": new_password}})
            return redirect('/')
        else:
    ...
    # Retrieve past orders for the current logged-in user
    email = session['user']['email']
    past_orders = db.ordermanagement_collection.find({'email': email})
    
    # Render the template with past orders data
    return render_template('past_orders.html', past_orders=past_orders)

@app.route('/auth', methods=['GET', 'POST'])
def auth():
    username = request.form.get('email')  # Access the request object directly
    ...
    if is_successful:
        session["user"] = user
        session["user_email"] = username  # Store the user's email in the session
        return redirect('/')
    else:
...
def past_orders():
    ...
    # Retrieve past orders for the current logged-in user from the database
    past_orders = list(db.ordermanagement_collection.find({'username': user_email}))
...
def api_get_products():
    ...
    resp = make_response(dumps(products))
...
def api_get_product(code):
    ...
    resp = make_response(dumps(product))
    resp.mimetype = 'application/json'
    return resp
