...
def get_products():
    return [{"code": code, "name": product["name"], "price": product["price"]} for code, product in products.items()]

def get_branch(code):
    return branches.get(code)

def get_branches():
    return [{"code": code, "name": branch["name"]} for code, branch in branches.items()]